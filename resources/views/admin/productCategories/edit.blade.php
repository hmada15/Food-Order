            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('parent_category') ? 'is-invalid' : '' }}" name="parent_category_id" id="parent_category_id">
                    @foreach($parent_categories as $id => $parent_category)
                        <option value="{{ $id }}" {{ (old('parent_category_id') ? old('parent_category_id') : $productCategory->parent_category->id ?? '') == $id ? 'selected' : '' }}>{{ $parent_category }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\ProductCategory::IS_PUBLISH_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('is_publish') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="is_publish_{{ $key }}" name="is_publish" value="{{ $key }}" {{ old('is_publish', $productCategory->is_publish) === (string) $key ? 'checked' : '' }} required>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.productCategory.fields.is_publish_helper') }}</span>
            </div>
            <div class="form-group">
